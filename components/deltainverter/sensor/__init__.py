...
from .. import CONF_DELTA_NAME, CONF_POSITION, CONF_LENGTH, DeltaInverter, deltainverter_ns

AUTO_LOAD = ["deltainverter"]

ValueFormat = deltainverter_ns.enum("ValueFormat")
VALUE_FORMAT_TYPES = {
    "float": ValueFormat.ValueFormat_Float,
    "hex": ValueFormat.ValueFormat_Hex,
    "int": ValueFormat.ValueFormat_Int
}
...
CONFIG_SCHEMA = sensor.sensor_schema().extend(
    {
        ...
        cv.GenerateID(CONF_ID): cv.use_id(DeltaInverter),
        cv.Required(CONF_DELTA_NAME): cv.string,
        cv.Required(CONF_POSITION): cv.int_,
        cv.Required(CONF_LENGTH): cv.int_,
        cv.Optional(CONF_FORMAT, default="int"): cv.enum(VALUE_FORMAT_TYPES, lower=True),
...
async def to_code(config):
    ...
    deltainverter = await cg.get_variable(config[CONF_ID])
